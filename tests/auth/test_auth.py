...
async def test_login_user(client: AsyncClient):
    ...
    await client.post("/auth/register", json={"email": email, "password": TEST_PASSWORD})
...
async def test_access_protected_route(client: AsyncClient):
    """Проверяем доступ к эндпоинту /auth/me с токеном."""
...
async def test_admin_route_forbidden_for_customer(client: AsyncClient):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = await client.get("/auth/admin-only", headers=headers)
