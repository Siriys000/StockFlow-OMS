...
*   *Зачем это нужно:* Создание первого суперюзера (admin) не следует выставлять в публичный REST API.
*   *Команды:*
    *   `create-admin <email> <password>`: Создает пользователя с ролью `ADMIN`.
*   *Пример запуска:*
    ```bash
    docker exec pyflow_app python src/cli.py create-admin admin@oms.com password123
    ```
